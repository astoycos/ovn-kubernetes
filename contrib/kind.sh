    ...
    while [ "$1" != "" ]; do
        case $1 in
            ...
            -hns | --host-network-namespace )   OVN_HOST_NETWORK_NAMESPACE=$1
                                                ;;
            -lkr | --local-kind-registry )      KIND_LOCAL_REGISTRY=true 
                                                ;;
            -dd | --dns-domain )                shift
                                                KIND_DNS_DOMAIN=$1
                                                ;;
...
set_default_params() {
  ...
  if [ ${KIND_CLUSTER_NAME} != "ovn" ]; then 
    MANIFEST_OUTPUT_DIR="../yaml/${KIND_CLUSTER_NAME}"
  fi 
...
coredns_patch() {
  ...
  echo $KUBECONFIG
  original_coredns=$(kubectl --kubeconfig=$KUBECONFIG get -oyaml -n=kube-system configmap/coredns)
...
# with the control-plane node's IP 
...
# if cluster name is set by user ensure it's an exactl match 
if [ $KIND_CLUSTER_NAME == "ovn"]; then 
  fixup_kubeconfig_names
fi 
