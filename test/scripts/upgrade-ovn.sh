...
# This script is responsible to upgrade ovn images and daemonsets from master to pr branch
KIND_CLUSTER_NAME=${KIND_CLUSTER_NAME:-ovn} # Set default values

# setup env needed for regenerating daemonsets from the pr branch
# this will use the new OVN image as well: ovn-daemonset-f:pr
set_default_ovn_manifest_params
print_ovn_manifest_params

# create upgraded OVN manifests from PR branch using existing configuration 
create_ovn_kube_manifests
install_ovn_image
...
# update ovnkube-master deployment
