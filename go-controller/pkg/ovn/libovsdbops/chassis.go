...
func DeleteStaleChassis(sbClient libovsdbclient.Client, chassisHostNameMap map[string]string) error {
	staleChassisNames := []string{}
	for _, v := range chassisHostNameMap {
		staleChassisNames = append(staleChassisNames, v)
	}

	if err := deleteChassis(sbClient, staleChassisNames...); err != nil {
		return err
	}
