		...
		if svcPort.NodePort > 0 {
			...
			for _, flowProtocol := range flowProtocols {
				...
				if epLocal {
					...
					klog.V(2).Infof("Updating to special ETP flows on br-ex for Nodeport Service %s", service.Name)
					// Rule 0:
					...
					if strings.Contains(flowProtocol, "6") {
						nodeportFlows = append(nodeportFlows,
							fmt.Sprintf("cookie=%s, priority=100, in_port=%s, %s, tp_dst=%d, actions=ct(commit,zone=%d,nat(dst=%s:%s),table=6)",
								cookie, npw.ofportPhys, flowProtocol, svcPort.NodePort, HostNodePortCTZone, npw.nodeIPs[1], svcPort.TargetPort.String()))
					} else {
						nodeportFlows = append(nodeportFlows,
							fmt.Sprintf("cookie=%s, priority=100, in_port=%s, %s, tp_dst=%d, actions=ct(commit,zone=%d,nat(dst=%s:%s),table=6)",
								cookie, npw.ofportPhys, flowProtocol, svcPort.NodePort, HostNodePortCTZone, npw.nodeIPs[0], svcPort.TargetPort.String()))
					}
					nodeportFlows = append(nodeportFlows,
						// Rule 1:
				...
				if strings.Contains(flowProtocol, "6") {
					nodeportFlows = append(nodeportFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, %s, %s=%s, tp_dst=%d, actions=ct(commit,zone=%d,nat(dst=%s:%s),table=6)",
							cookie, npw.ofportPhys, flowProtocol, nwDst, ing.IP, svcPort.Port, HostNodePortCTZone, npw.nodeIPs[1], svcPort.TargetPort.String()))
				} else {
					nodeportFlows = append(nodeportFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, %s, %s=%s, tp_dst=%d, actions=ct(commit,zone=%d,nat(dst=%s:%s),table=6)",
							cookie, npw.ofportPhys, flowProtocol, nwDst, ing.IP, svcPort.Port, HostNodePortCTZone, npw.nodeIPs[0], svcPort.TargetPort.String()))
				}
				nodeportFlows = append(nodeportFlows,
					// Rule 1:
					// Sends the packet to the host's networking stack after rule 1 is done DNATing
		...
		for _, externalIP := range service.Spec.ExternalIPs {
			...
			if epLocal {
				...
				if strings.Contains(flowProtocol, "6") {
					nodeportFlows = append(nodeportFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, %s, %s=%s, tp_dst=%d, actions=ct(commit,zone=%d,nat(dst=%s:%s),table=6)",
							cookie, npw.ofportPhys, flowProtocol, nwDst, externalIP, svcPort.Port, HostNodePortCTZone, npw.nodeIPs[1], svcPort.TargetPort.String()))
				} else {
					nodeportFlows = append(nodeportFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, %s, %s=%s, tp_dst=%d, actions=ct(commit,zone=%d,nat(dst=%s:%s),table=6)",
							cookie, npw.ofportPhys, flowProtocol, nwDst, externalIP, svcPort.Port, HostNodePortCTZone, npw.nodeIPs[0], svcPort.TargetPort.String()))
				}
				nodeportFlows = append(nodeportFlows,
					// Rule 1:
					// Sends the packet to the host's networking stack after rule 1 is done DNATing
...
func (npw *nodePortWatcher) AddEndpoints(ep *kapi.Endpoints) {
	...
	if _, exists := npw.services[name]; exists {
		...
		if util.ServiceExternalTrafficPolicyLocal(npw.services[name]) {
			...
			if countHostNeworkedEndpoints(ep, npw.nodeName) > 0 {
				klog.V(5).Infof("Service %s has ETP=local and local Endpoints, Updating flow Cache", ep.Name)
...
func (npw *nodePortWatcher) DeleteEndpoints(ep *kapi.Endpoints) {
	...
	if _, exists := npw.services[name]; exists {
		if util.ServiceExternalTrafficPolicyLocal(npw.services[name]) {
			if countHostNeworkedEndpoints(ep, npw.nodeName) == 0 {
				klog.V(5).Infof("Service %s has ETP=local and no local Endpoints, Updating flow Cache", ep.Name)
	...
	// We add this only when a Service event is received, delete only when endpoint delete event is received
...
func (npw *nodePortWatcher) UpdateEndpoints(old, new *kapi.Endpoints) {
	...
	if _, exists := npw.services[name]; exists {
		if util.ServiceExternalTrafficPolicyLocal(npw.services[name]) {
			...
			if countHostNeworkedEndpoints(old, npw.nodeName) > 0 && countHostNeworkedEndpoints(new, npw.nodeName) == 0 {
				klog.V(5).Infof("Service %s has ETP=local, had local Endpoints, but now does not Updating flow Cache", new.Name)
				// Delete ETP flows
				npw.updateServiceFlowCache(npw.services[name], false, true)
				needFlowSync = true
				...
				if len(new.Subsets) > 0 {
					klog.V(5).Infof("Service %s has ETP=local, had local Endpoints, and still has normal ones Updating flow Cache", new.Name)
			...
			if countHostNeworkedEndpoints(old, npw.nodeName) == 0 && countHostNeworkedEndpoints(new, npw.nodeName) > 0 {
				...
				klog.V(5).Infof("Service %s has ETP=local and didn't have local Endpoints but now does Updating flow Cache", new.Name)
				npw.updateServiceFlowCache(npw.services[name], true, true)
				needFlowSync = true
