...
func addSharedGatewayIptRules(service *kapi.Service, nodeLocalHostEndpoint bool, nodeIPs []string) {
	...
	if nodeLocalHostEndpoint {
		rules = getGatewayIPTRules(service, nodeLocalHostEndpoint, nodeIPs)
	} else {
		rules = getGatewayIPTRules(service, nodeLocalHostEndpoint, nil)
	}
...
func syncSharedGatewayIptRules(services []interface{}, nodeLocalHostEndpoint bool, nodeIPs []string) {
	...
	for _, service := range services {
		...
		if nodeLocalHostEndpoint {
			keepIPTRules = append(keepIPTRules, getGatewayIPTRules(svc, nodeLocalHostEndpoint, nodeIPs)...)
		} else {
			keepIPTRules = append(keepIPTRules, getGatewayIPTRules(svc, nodeLocalHostEndpoint, nil)...)
		}
	}
